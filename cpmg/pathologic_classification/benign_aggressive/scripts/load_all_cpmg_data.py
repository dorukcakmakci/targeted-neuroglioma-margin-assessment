...
datapath_base = os.path.join(project_base_path, "data/cpmg/raw/")
...
valid_quant_availability = c_quantification_availability[valid_sample_indices, :]
...
fold_dct, class_labels = split_to_kfold(
    spectra, statistics, "benign_aggressive", k=5, seed=SEED
)
class_labels = np.array(class_labels).reshape(-1, 1)

non_control_indices = list(np.where(class_labels == 1)[0]) + list(
    np.where(class_labels == 0)[0]
)
...
quant_availability = quant_availability[non_control_indices, :]
...
all_v_ppm_spectra = ppm_spectra
all_v_spectra = scaled_spectra
all_v_statistics = statistics
all_v_quant = quant
all_v_quant_availability = quant_availability
all_v_class_labels = class_labels
all_v_metabolite_names = metabolite_names
all_v_fold_dct = fold_dct
