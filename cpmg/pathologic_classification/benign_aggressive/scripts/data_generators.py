...
def cpmg_generator_1A(
    k, fold_dct, statistics, spectra, ppm_spectra, quant, class_labels
):
    cur_iter = 0
    while cur_iter < k:
        ...
        train_fold_idx = (
            []
            + fold_dct[str(train_fold_indices[0])]
            + fold_dct[str(train_fold_indices[1])]
            + fold_dct[str(train_fold_indices[2])]
        )
        train_fold = {}
...
def cpmg_generator_1B(
    ...
    while cur_iter < k:
        ...
        train_fold_idx = (
            ...
            + fold_dct[str(train_fold_indices[1])]
            + fold_dct[str(train_fold_indices[2])]
            + fold_dct[str(train_fold_indices[3])]
        )
        train_fold = {}
        train_fold["spectra"] = spectra[train_fold_idx, :]
        train_fold["quant"] = quant[train_fold_idx, :]
...
def cpmg_generator_2A(
    ...
    while cur_iter < k:
        ...
        train_fold = {}
        train_fold["spectra"] = np.concat(
            (spectra[train_fold_idx, :], invalid_spectra[:, :]), axis=0
        )
        train_fold["quant"] = np.concat(
            (quant[train_fold_idx, :], invalid_quant[:, :]), axis=0
        )
        train_fold["ppm_spectra"] = np.concat(
            (ppm_spectra[train_fold_idx, :], invalid_ppm_spectra[:, :]), axis=0
        )
        train_fold["class_labels"] = np.concat(
            (
                class_labels[train_fold_idx, :],
                np.array([-1] * invalid_sample_cnt).reshape((-1, 1)),
            ),
            axis=1,
        )
        train_fold["stats"] = pd.concat(
            [statistics.iloc[train_fold_idx, :], invalid_statistics]
...
def cpmg_generator_4A(
    ...
    valid_ppm_spectra,
    valid_quant,
    valid_quant_availability,
    valid_class_labels,
    invalid_statistics,
    invalid_spectra,
    invalid_ppm_spectra,
    invalid_quant,
    invalid_quant_availability,
):
    cur_iter = 0
    while cur_iter < k:
        ...
        test_fold["stats"] = valid_statistics.iloc[test_fold_idx, :].reset_index(
            drop=True
        )
        ...
        vald_fold["stats"] = valid_statistics.iloc[vald_fold_idx, :].reset_index(
            drop=True
        )
        ...
        train_fold = {}
        train_fold["spectra"] = np.concat(
            (valid_spectra[train_fold_idx, :], invalid_spectra[:, :]), axis=1
        )
        train_fold["quant"] = np.concat(
            (valid_quant[train_fold_idx, :], invalid_quant[:, :]), axis=1
        )
        train_fold["quant_availability"] = np.concat(
            (
                valid_quant_availability[train_fold_idx, :],
                invalid_quant_availability[:, :],
            ),
            axis=1,
        )
        train_fold["ppm_spectra"] = np.concat(
            (valid_ppm_spectra[train_fold_idx, :], invalid_ppm_spectra[:, :]), axis=1
        )
        train_fold["class_labels"] = np.concat(
            (
                valid_class_labels[train_fold_idx, :],
                np.array([-1] * invalid_sample_cnt).reshape((-1, 1)),
            ),
            axis=1,
        )
        train_fold["stats"] = pd.concat(
            [
                valid_statistics.iloc[train_fold_idx, :].reset_index(drop=True),
                invalid_statistics,
            ]
        )
