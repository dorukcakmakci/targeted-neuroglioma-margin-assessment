...
ppm_region_df = pd.DataFrame(
    dict([(k, pd.Series(v)) for k, v in dataset2ppmregion.items()])
)
spectrum_region_df = pd.DataFrame(
    dict([(k, pd.Series(v)) for k, v in dataset2spectrumregion.items()])
)

writer = pd.ExcelWriter("./metabolite_regions.xlsx", engine="xlsxwriter")
spectrum_region_df.transpose().to_excel(
    writer,
    sheet_name="Metabolite Regions (Spectrum)",
    index=dataset2spectrumregion.keys(),
)
ppm_region_df.transpose().to_excel(
    writer, sheet_name="Metabolite Regions (PPM)", index=dataset2ppmregion.keys()
)
writer.save()
pdb.set_trace()
