...
target_dataset_excel_path = os.path.join(
    project_base_path, "data/cpmg/cpmg_final_dataset.xlsx"
)
with open("./valid_cpmg_sample_filenames", "rb") as f:
...
statistics_dataset = statistics.drop(
    labels=[
        "Availability",
        "Recidive",
        "Tumor Location",
        ...
        "Metabolite Quantification Filename",
    ],
    axis="columns",
)
...
statistics_dataset.to_excel(
    writer,
    sheet_name="ERETIC-CPMG Dataset Statistics",
    na_rep="*",
    header=True,
    index=True,
)
quant.to_excel(
    writer,
    sheet_name="ERETIC-CPMG Dataset Metabolites",
    na_rep="*",
    header=True,
    index=True,
)
