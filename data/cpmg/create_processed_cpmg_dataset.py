...
dataset_excel_path = os.path.join(project_base_path, "data/all_cpmg_hrmas_dataset.xlsx")
dataset_spectra_path = os.path.join(project_base_path, "data/cpmg_dataset/")
raw_data_path = os.path.join(project_base_path, "data/cpmg/raw/")

# read excel sheets to pandas dataframe
statistics_df = pd.read_excel(dataset_excel_path, sheet_name="Tissue Sample Collection")
quantification_df = pd.read_excel(
    dataset_excel_path, sheet_name="Metabolite Quantifications"
)
statistics_df["Pathologic Classification"] = statistics_df[
    "Pathologic Classification"
].replace("Control ", "Control")

# read metabolite quantifications
metabolite_names = quantification_df.columns[4:]
quantification_df[metabolite_names] = quantification_df[metabolite_names].replace(
    "*", "0"
)
...
for (c_name, _) in quantification_availability.iteritems():
    quantification_availability[c_name] = quantification_availability[c_name].apply(
        condition
    )
...
spectra = np.array(
    [
        preprocess_spectrum(f"{dataset_spectra_path}{x}/4/").astype(float)
        if os.path.exists(f"{dataset_spectra_path}{x}/4/")
        else np.array([0] * 16314)
        for x in spectra_filenames
    ]
)

# separate eretic and cpmg data
cpmg_idx = statistics_df.index[statistics_df["Spectrum Type"] == "CPMG"].tolist()
cpmg_statistics = statistics_df[statistics_df.index.isin(cpmg_idx)].reset_index(
    drop=True
)
...
eretic_idx = statistics_df.index[
    statistics_df["Spectrum Type"] == "Eretic-CPMG"
].tolist()
eretic_statistics = statistics_df[statistics_df.index.isin(eretic_idx)].reset_index(
    drop=True
)
eretic_quantifications = metabolite_quantifications[eretic_idx]
eretic_quantification_availability = quantification_availability[eretic_idx]
eretic_spectra = spectra[eretic_idx]
...
for pid in cpmg_patient_ids:
    ...
    if len(test_sid) > 0:
        ...
        pos_indices = list(
            cpmg_statistics.loc[test_sid, :]
            .loc[cpmg_statistics.Pathology.isin(["pos"]), "Pathologic Classification"]
            .index
        )
        neg_indices = list(
            cpmg_statistics.loc[test_sid, :]
            .loc[cpmg_statistics.Pathology.isin(["neg"]), "Pathologic Classification"]
            .index
        )
        cpmg_statistics.loc[
            pos_indices, "Pathologic Classification"
        ] = glioma_pathologic_label
...
# save cpmg data
with open(f"{raw_data_path}all_samples_spectra", "wb") as f:
    pickle.dump(cpmg_spectra, f)
with open(f"{raw_data_path}all_samples_quantification", "wb") as f:
...
cpmg_statistics.to_pickle(f"{raw_data_path}all_samples_statistics")
