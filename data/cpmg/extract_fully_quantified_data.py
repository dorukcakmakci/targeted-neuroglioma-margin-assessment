...
source_data_path = os.path.join(project_base_path, "data/cpmg/raw/")
target_data_path = os.path.join(project_base_path, "data/cpmg/fully_quantified/")
...
with open(
    os.path.join(source_data_path, "all_samples_quantification_availability"), "rb"
) as f:
    all_quantification_availability = pickle.load(f)
all_statistics = pd.read_pickle(
    os.path.join(source_data_path, "all_samples_statistics")
)


# generate target data
fully_quantified_idx = np.where(np.mean(all_quantification_availability, axis=1) == 1)[
    0
]
...
fully_quantified_samples_statistics = all_statistics.iloc[
    fully_quantified_idx
...
fully_quantified_samples_statistics.to_pickle(
    f"{target_data_path}fully_quantified_samples_statistics"
)
