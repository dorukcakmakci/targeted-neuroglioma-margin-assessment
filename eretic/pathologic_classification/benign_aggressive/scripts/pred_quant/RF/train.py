...
for fold_id, (
    ...
    generator(K, fold_dct, statistics, spectra, ppm_spectra, pred_quant, class_labels)
):
    print(f"Test fold {fold_id}")

    train_data["X"] = train_data["quant"]
    vald_data["X"] = vald_data["quant"]
    test_data["X"] = test_data["quant"]
    ...
    figure.savefig(
        os.path.join(plot_base_path, f"test_fold_{fold_id}_tumor_shap_plot.pdf")
    )
