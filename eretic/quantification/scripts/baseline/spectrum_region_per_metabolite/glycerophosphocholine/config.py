...
from metabolite_mapping import dataset2folder, folder2dataset, folder2spectrumregion 

# task configuration
task = "glycerophosphocholine"
dataset_task = folder2dataset[task]
task_target_idx = fq_v_metabolite_names.index(dataset_task)
task_feature_idx = folder2spectrumregion[task] ##################

# data configuration
ppm_spectra = fq_v_spectra[:, task_feature_idx] ###################
spectra = fq_v_ppm_spectra
...
model_name = f"baseline/spectrum_region_network_per_metabolite/{task}/seed_{SEED}/" ############
