...
for outer_idx, (
    ...
    # normalize losses with respect to maximum quantification per metabolite
    max_quant_values_in_cohort = torch.from_numpy(np.mean(train_data["quant"], axis=0))
    ...
    vald_dataloader = DataLoader(
        vald_dataset, batch_size=len(vald_dataset), shuffle=True
    )
    ...
    model = Model(len(metabolite_names)).to(device)
    model.apply(initialize_weights)
    optimizer = torch.optim.Adam(
        model.parameters(), lr=hp_space["ETA"], weight_decay=hp_space["weight_decay"]
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (_, ppm_spectra, target, _) in enumerate(train_dataloader):
            ...
            optimizer.zero_grad()
            preds = list(model(ppm_spectra))
            loss = None
            for idx, metabolite in enumerate(metabolite_names):
                temp = (
                    loss_fn_1(preds[idx], target[:, idx].squeeze())
                    / max_quant_values_in_cohort[idx]
                )
        ...
        if epoch % 5 == 0:
            print(
                f"Epoch: {epoch}\tTrain Loss (MSE): {total_train_loss}\tValidation Loss (MSE): {total_vald_loss}"
            )
        # early stopping
    ...
    test_preds = []
    for idx, metabolite in enumerate(metabolite_names):
        test_preds.append(preds[idx].detach().cpu().numpy())
    test_label = test_label.detach().cpu().numpy()
    test_pred = np.vstack(tuple(test_preds)).T
    ...
    mape_ = mean_absolute_percentage_error(
        test_label, test_pred, multioutput="raw_values"
    )
    ...
    for idx, metabolite in enumerate(metabolite_names):
        ...
        metrics["absolute_percentage_error"][
            metabolite
        ] += absolute_percent_error_per_sample[:, idx].tolist()
...
for metabolite_ind, metabolite in enumerate(metabolite_names):
    with open(
        os.path.join(log_base_path, f"{metabolite}__absolute_percentage_error.txt"), "w"
    ) as f:
