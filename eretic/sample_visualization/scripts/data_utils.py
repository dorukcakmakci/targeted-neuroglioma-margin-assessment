...
def get_distribution(labels):
    ...
    return [
        f"{label_distribution[i] / sum_labels:.2%}" for i in range(np.max(labels) + 1)
    ]
...
def stratified_group_k_fold(x, y, groups, k, seed=None):
    ...
    def eval_label_counts_per_fold(label_counts, fold):
        ...
        for label in range(label_count):
            label_std = np.std(
                [
                    label_counts_per_fold[i][label] / label_distribution[label]
                    for i in range(k)
                ]
            )
    ...
    for group, label_counts in sorted(
        groups_and_label_counts, key=lambda x: -np.std(x[1])
    ):
        best_fold = None
        min_eval = None
        for i in range(k):
...
def split_to_kfold(spectra, statistics, task, k=5, seed=35):
    ...
    for fold, (train_idx, test_idx) in enumerate(
        stratified_group_k_fold(spectra, label, patient_id, k=k, seed=seed)
    ):
...
def spectrum2ppm(spectra):
    ppm_spectra = np.zeros(
        (spectra.shape[0], int((UPPER_PPM_BOUND - LOWER_PPM_BOUND + STEP) / STEP))
    )
    for ppm in np.arange(LOWER_PPM_BOUND / STEP, (UPPER_PPM_BOUND + STEP) / STEP, 1):
        ...
        ppm_spectra[:, int((ppm - LOWER_PPM_BOUND) / STEP)] = np.sum(
            spectra[:, idx_range], axis=1
        )
    return ppm_spectra
